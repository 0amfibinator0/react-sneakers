...
function Drawer({onClose, onRemove, items = [], opened }) {
  ...
  const onClickOrder = async () => {
    try {
      setIsLoading(true)
      const {data} = await axios.post('https://62e16e35fa99731d75d66e03.mockapi.io/orders', {
        items: cartItems,
      });
      setOrderId(data.id);
      ...
      for (let i = 0; i < cartItems.length; i++) {
        ...
        await axios.delete('https://62e16e35fa99731d75d66e03.mockapi.io/cart/' + item.id);
        await delay(1000);
      }
  ...
  return (
    <div className={`${styles.overlay} ${opened ? styles.overlayVisible : ''}`}>
      <div className={styles.drawer}>
        ...
        {items.length > 0 ? (
          <div className="parent d-flex flex-column flex">
